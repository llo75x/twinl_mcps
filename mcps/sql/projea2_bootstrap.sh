...
COL_MANQ=$(q "SELECT COUNT(*) FROM information_schema.columns c
               WHERE c.table_schema='projea2'
                 ...
                 AND NOT (
                   ...
                   (c.table_name='tracking_links'   AND c.column_name='token') OR
                   (c.table_name='export_approvals' AND c.column_name='token') OR
                   (c.table_name='tracking_hits'    AND c.column_name='ip'))
...
for v in NB_VUES FUITE_COL FUITE_TBL COL_MANQ SANS_VUE COLLISION APP_OK; do
